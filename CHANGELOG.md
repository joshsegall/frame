...
### Fixed
- `fr mv` can now move a task that lives in the Parked or Done section, not just the Backlog. Previously a cross-track move or reorder of a completed task failed with `task not found` even though `fr show` resolved it, because the move only scanned the Backlog. The task now moves and keeps its state — a moved Done task lands in the target track's Done section with its `resolved:` date intact, rather than being silently reopened. (The actor-token id form like `TOO-b8` was a red herring; the failure was the section, not the token.)
