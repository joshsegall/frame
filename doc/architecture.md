...
**`note:` is still one opaque value.** It is the field both sides touch most and the one whose merge would pay best, but merging it properly means a three-way merge of its *lines*, with a failure mode the others do not have: text nobody wrote, landing mid-rebase. Until that exists, a note both sides changed differently conflicts exactly as it did before — and the rest of the task now merges around it.
