...
So archives merge as `FileKind::Archive`, through `reconcile::reconcile_archive_tasks` — the same identity merge over a flat list rather than over sections. It is a *simpler* merge than a track's: no relocation to reconcile, and position carries no meaning, so both sides' appends are a union. Our header, our trailing content and our line ending are carried across; theirs are not merged, because frame does not understand either and prose is what a VCS already handles better.
