...
- **Cross-track move**: Rewrites the moved task's ID (and all subtask IDs) to the target track's prefix, then scans **all** tracks to update dep references pointing to any of the old IDs
- **A renumbering rewrites what points at *every* id it changed**: a subtree's renaming is a map, not a pair, and each of the three operations that renumber (cross-track move, reparent, `fr check --fix`'s parent-extend) hands its whole map to `task_ops::apply_id_map_to_deps`. Applying the map one pair at a time would let a dep rewritten by one pair be rewritten again by a later one, so it is matched once against the pre-image and applied in a single pass. Reporting only the root rename is what left a dep on a moved subtask dangling — including one from a sibling inside the same subtree, which moved along with the task it depends on. `fr check --fix` will not repair a dangling dep (dropping it discards intent), so the operation that creates the rename is the only place this can be got right.
- **A dep resolves against the archives as well as the live tracks**: an archive holds done work, so a `dep:` whose target was archived is *satisfied*, not missing — `ops::deps::ArchiveIndex` is the one reader, shared by `fr deps`, `fr check`, `fr clean` and the TUI's dep popup, and it loads on first use so the common case never opens an archive. Without it, archiving invented errors: `fr clean` drains a Done section past the threshold, every dep on a drained task leaves the live id set, and the *next* `fr check` reports a project nobody touched. `tests/conservation.rs` had read the archives as part of the project since it was written (claim 5, "every dep that resolved still resolves"), so until this the repo held two definitions of "resolves" and the product had the worse one.
- **Every track the rewrite touches gets written**: the rewrite reaches tracks the operation itself never moved anything into or out of, and those have to be saved too. Both surfaces select them with `task_ops::track_has_dirty_task` — after the in-flight marker commits, since the move is already durable once source and target are on disk and a failure here leaves a recoverable dangling dep rather than a half-moved task.
...
**An `unsaved` entry names a file whose content this session still holds.** That is what makes the retry meaningful, what `dump_unsaved` copies into `.rescue/` at exit, and what stops a reload from overwriting the only copy there is. An entry naming a track no longer in `project.tracks` satisfies none of the three: the retry re-runs the same lookup and fails the same way, the rescue has nothing to write, and — because `is_permanent` matches "not found" — the timer never tries at all. The indicator stays lit for the rest of the session naming a file nobody can produce, and the exit report marks it `[NO RESCUE COPY — this one is gone]` when there was never anything to copy.

So a track leaves memory through one door, `App::release_track`, the counterpart to `adopt_track_from_disk`, and the entry is resolved on the way out: flushed if the file is still ours, and otherwise written to the recovery log, which is where content that reached no other file goes. Not `.rescue/` — that is a set of files meant to be moved back into place, and putting `tracks/<id>.md` back is exactly what must not happen to a track that was just archived or deleted. **The flush has to precede the file move**: `track_file` still resolves to `tracks/<id>.md` for an archived row, so flushing afterwards writes the track back into the directory the archive just took it out of. And "nothing to save" is not a failed save — a save asked for a track this session does not hold is reported to the recovery log, loudly, but never put on the books where nothing could ever take it off.
...
**An absorbed version becomes the ancestor.** Whichever way a merge went, that version has been dealt with, so it is what memory and disk last agreed on — on the reload path *and* the save path, on the plain replace path where no merge ran at all, and on the paths where a merge is *avoided*: a handler that sees the file changed under it (`track_changed_on_disk`) and keeps what is there has dealt with that version as surely as a merge would have. Eleven such sites updated only the mtime, and P8 found what that costs — a title the CLI had written and acknowledged came back as the version before it. `App::adopt_track_from_disk` is the one way to take a file as ours; `replace_track` stays the raw primitive for the merged track that no file holds. Leaving the old ancestor in place makes the next merge meet the same external change a second time, and by then our copy contains it, so the merge reads it as a task both sides edited and files the other version in the recovery log as a conflict nobody was in dispute over.
...
**The recovery log holds contested versions only.** The last bullet is the merge's sole route into it, so a version landing there is a claim that two writers changed one task and only one could survive. If a task's version reaches the log without the session ever having touched that task, the merge did not resolve a conflict — it lost an update, and the inputs were wrong before it ran (the ancestor rule above is how they go wrong). This is the contrapositive of that rule and the sharper thing to test for: P8 asserts it directly, because "one side survives and the other is logged" is satisfied *precisely* by that defect.
...
**Unmodelled content is anchored to its predecessor, in both formats.** `Task::trailing_lines` and `InboxItem::trailing_lines` carry lines frame preserves but does not understand, and both anchor to the node *above* — a lesson learned the hard way on the track side, where anchoring to the successor put a line inside one task's subtree while a different task carried it, so a section move left it behind to be read as somebody else's note and destroyed. Every stranded line has a predecessor: in the inbox, a line before the first item is header rather than stranded, which makes the anchor total.

For the inbox the *blank line* is part of what gets carried, and that is not cosmetic. A line directly under an item is body text; the same line with a blank above it belongs to no item. Re-emitting a preserved line without its blank would silently change what it means on the next read — a worse outcome than the drop it replaced. The corollary is that re-anchoring appends verbatim and may leave one blank more than needed: erring toward more blanks is safe and stable, erring toward fewer changes meaning, and a pure append is also the exact inverse undo needs (`Inbox::take_item` / `restore_item`).

Because the anchor can go away, every path that removes an inbox item for good — triage, delete, and a merge that applies another writer's deletion — re-anchors what it was carrying rather than taking it along. P7 catches a path that forgets, on its first generated operation.
...
**Adopting a config is not a re-init.** The reload path used to skip `project.toml` entirely as "would need full re-init" — which would throw away the undo stack, every track's view state, and anything sitting in `unsaved`. `App::adopt_config` instead parses in tracks that appeared, releases tracks that went away, and re-resolves `View::Track` by track id rather than by index. Releasing them does *not* flush: a track is gone from the config because its row was removed or turned archived, so `tracks/<file>` is no longer ours to write, and the flush that used to happen here recreated the file another process had just deleted or moved. Loading someone else's track is a passive load and does not mint.

**Everything a config decides is decided in that one place, including the theme.** The rebuild used to sit at the call site that takes a config from disk *whole*, so the two that adopt a **merged** one — a reload while the session holds an unsaved config change, and a save that takes the other writer's keys along with ours — left the old palette on screen with the new colours sitting in `project.config`. Those two are reached only when a second writer is active, which is the only time the question arises. A derived view of the config belongs behind the adoption, not beside one of its callers.
...
**Tasks are not the whole file, and treating them as one was the second silent loss.** `parse_track` puts every line in exactly one of two places: a task, or a `TrackNode::Literal` / a section's header and trailing lines. `rebuild` used to start from `ours.clone()` and refill nothing but `Section.tasks`, so everything in the second place — the `# Title` line, the `> description`, prose between sections, and any `##` heading frame does not model — came from our side *unconditionally*. A merge whose only change was on their side of one of those returned our file byte for byte, reported no conflict, and exited 0. It was reported after nearly taking a repair to a heading with it, on a rebase, where "ours" was upstream and the repair was the reader's own.

So `reconcile::track_shell` names that second place and it merges by the same rule the tasks get: whoever changed it wins, and if both did it is a conflict, ours kept and theirs in the recovery log. Whichever side wins supplies the structure `rebuild` fills, which is how their version reaches the file without a second serializer. Comparison drops blank lines (`shell_signature`) because **tasks move them** — adding the first task under a bare `## Done` pulls the blank beneath the header into that section's `header_lines`, and comparing verbatim would conflict two writers for doing the one thing the merge exists to allow. An archive's header and trailing text go the same way; the inbox's header cannot conflict, so a line the losing side added is carried instead, which is the multiset answer its items already get.

**A shell conflict names no task, so it leaves no marker.** There is nothing in the file to hang one on, and `fr check` and `fr merge --resolve` both walk `project.tracks` by task. The non-zero exit carries it instead — the same trade an archive conflict makes, for the same reason — and the report says so plainly rather than pointing at a `conflict:` line that is not there.

**The audit, because the fix for a class should not be one more instance.** Both silent losses were invisible for the same reason: nothing ever asked whether the result still held what it was given. `merge_files::unaccounted_additions` asks it. A line missing from the result is usually right — we deleted its task, or we edited it and they did not — but a line that is **not in the ancestor** cannot have been deleted by anyone, so an addition that vanished is a loss with no decision behind it. On an otherwise-clean merge, one turns into a conflict and a halt. It runs only when nothing else was reported, because a conflicted task is *supposed* to lose one side's lines. `tests/merge_conservation.rs` states the same property over generated edits, and treats the audit firing as the failure rather than the excuse.

**"Ours" is not a side, it is a label, and it inverts.** Under `git merge` ours is the branch checked out. Under `git rebase` HEAD is the branch being replayed *onto*, so ours is upstream and your own commit is theirs — and `git cherry-pick` and `git revert` invert the same way. A driver is never told which it is in, so `io::git::operation_kind` looks: `rebase-merge`/`rebase-apply` and `sequencer/` exist for the whole operation and are visible, while `MERGE_HEAD` and `CHERRY_PICK_HEAD` are written *after* the strategy runs and are not (measured, not assumed). That asymmetry is the useful one — the operations that invert the sides are the detectable ones — and where nothing is detected the report states the rule rather than assuming the reader's side. The old report ended on *"the merged file is valid frame markdown — no conflict markers were written"*, which is true, is the most quotable line in the output, and sat directly under the news that a version had been discarded.

### An archive is a third shape, and the driver has to know it

`frame/archive/<track>.md` is a **flat task list** with no `## Section` headers, so it is neither a track nor the inbox. `kind_for_path` used to return `Track` for anything under a directory named `archive`, and the consequence was not a cosmetic misread: `parse_track` finds no section header, collapses the whole file into one `TrackNode::Literal`, and `reconcile` indexes **zero tasks** on both sides. With no keys there are no outcomes and no conflicts, and `rebuild` starts from ours — so the driver returned our file verbatim, discarded every task only they had archived, and exited 0. Two clones that had each run `fr clean` since the common ancestor silently lost one side's archived tasks, with `fr check` reporting the project valid afterwards.

The driver was, on that one path, **worse than no driver at all**: git's own text merge leaves a conflict with both sides' tasks visible in the file, which a human then resolves. Losing to the fallback is the sharpest possible statement that a special case was wrong.

So archives merge as `FileKind::Archive`, through `reconcile::reconcile_archive_tasks` — the same identity merge over a flat list rather than over sections. It is a *simpler* merge than a track's: no relocation to reconcile, and position carries no meaning, so both sides' appends are a union. The header and the trailing content take the shell rule above; the line ending stays ours, because it is a property of the file being written rather than of the text that went into it.

**An archive conflict carries no marker.** `fr check`'s `UnresolvedMergeConflict` detector runs from `check_track` over `project.tracks`, and `fr merge --resolve` iterates the same — neither sees archive files, so a marker written into an archive could be neither reported nor cleared. A marker exists to carry an unmade decision to another clone or a later session; an archive conflict has no such audience, because the non-zero exit puts it in front of the only person who can decide. Extending `check` and `--resolve` to archives to make markers work would be paying for an audience that does not exist.

**The general lesson, which is the part worth keeping.** Two independent things decide how a file is merged: `.gitattributes` decides what git hands the driver, and `kind_for_path` decides how the driver reads it. Nothing structural kept them in agreement, and a catch-all in the second one turned a disagreement into a silent misparse. `every_routed_pattern_has_a_kind` now derives the patterns from `git_setup::attribute_lines` and asserts a kind for each, so a newly routed shape fails the build until someone decides what it parses as. **A file shape with no rule declines; it never gets a default.**
...
### A conflict is per field, not per task

`decide` used to answer for a whole task, and `same_content` compared state, title, tags, the entire metadata vec and the stranded-line fields as one value. So *any* two divergent edits to one task conflicted however unrelated they were — a `resolved:` filled by us against a note appended by them, a state change against a `ref:`. In a sample of real conflicts from a project running several concurrent writers, two thirds were that shape, and the merge had nothing to adjudicate in any of them.

A task is a record of independent fields, so each gets the ordinary three-way rule on its own: changed on one side only, take that side; changed identically on both, take it; changed divergently, conflict. Only a field that actually diverged makes the task conflict, and `Outcome::Merged` carries the recombined task. This was never a new idea in the module — `reconcile_config` has merged `project.toml` per field since it was written. The two halves of one file simply disagreed about what a merge unit is.

**`ref:` and `spec:` are sets, and therefore never conflict.** Both are documented as comma-separated file paths and are already `Vec<String>`. Two sides each adding paths is a union; a removal is honoured against the other side's additions. There is no "edit an element" for a set — an edit is a removal plus an addition — so there is no divergence left for these two to have. That alone resolves the case where both sides made the *identical* edit, which is what one investigation graduating to a new path looks like from two clones.

**`note:` merges by its lines.** It is the field two writers touch most, and notes are append-structured by convention: dated episodes accumulate at the tail while earlier sections stay put. So the dominant shape of a note both sides changed is *append at the end* against *edit far above* — two changes with nothing to disagree about, which as one opaque value were a conflict every time. `similar`'s three-way line merge resolves them; a conflict now means the changed line ranges actually overlap.

Three things keep that from being the risky change it sounds like. **No markers can reach the file**: `similar` can render `<<<<<<<`, and a note carrying those would make the file unparseable, so the marker path is never called — the result is assembled from the resolved regions and a conflicted merge returns before any rendering. **Every line comes from one of the two writers**, so the merge invents no text and stays transparent to the audit below: a merged note holds both sides' added lines verbatim and nothing has to be excused for it. And **it declines rather than guesses**: unless both sides carry exactly one note it falls back to the whole-value rule, because a task with two `- note:` lines has two notes and pairing them off by position would splice text that was never the same text.

**The section is a field too, and it is the one `merge_fields` does not hold.** Placement is decided separately by `section_for`, which prefers ours when both sides moved a task. Left alone, two sides moving one task to *different* sections would compose silently into ours, so `decide` checks for it explicitly.

**A field-merged task is synthesized, so it is dirty.** Neither side's stored source lines describe it, so it is re-emitted from the model — in canonical field order, with canonical spacing. A merge can therefore produce a task that differs from *both* sides in layout as well as content. That is the same rewrite any edited task already gets; it is new only for the merge path.

**What it costs the audit, and what pays for that.** `unaccounted_additions` asks whether an addition of theirs vanished *without a decision behind it*, reading the result line by line. That premise held exactly while a task was the unit of decision: the result took one side's task lines verbatim or the other's. A field-merged task breaks it honestly — its task line is built from both sides, so theirs' version is absent by decision, and a line-level reading cannot tell that from a loss. So `reconcile::Trace::superseded` names the lines the merge decided about and the audit subtracts them.

That is a hole in the audit only if the merge can supersede a line without deciding about it. What rules that out is `merge_fields` destructuring `Task` exhaustively: it decides every field or it fails the build, so a new field cannot be silently taken from ours while the audit is told to look away. Same move as `Metadata::rank` and `every_routed_pattern_has_a_kind` — where a guard's soundness rests on a list being complete, the compiler keeps the list complete.

`tests/merge_conservation.rs` states the same exception independently and narrowly: only a task line, only where the other side changed that line too, only where the task is still in the result. Every metadata line still stands on its own, so a dropped `resolved:` or `ref:` fails there.
...
### One barrier under every write

`--dry-run` is a promise about *all* of frame's writes, so it is enforced in one place rather than by each command. `io::dryrun` holds a flag armed once at the top of a handler, and every write entry point asks it before touching the filesystem: `recovery::atomic_write` (which every track, archive, inbox, config, ID-frontier and actor-registry write goes through), the recovery-log append, the in-flight marker, and the handful of renames, removals and directory creations that are not `atomic_write` — `io::dryrun` wraps those as `write` / `rename` / `remove_file` / `create_dir_all`, so a new write site gets the barrier by using them.

**The alternative is what frame did, and it is why this exists.** Six commands had `--dry-run`, each guarding its own saves. `fr clean --dry-run` guarded the archive write and documented itself as writing nothing, while the ID mint underneath went on calling `io::ids::reserve` — advancing the durable frontier, in a store shared by every worktree of the clone, once per previewed ID. A guard per command can only cover the writes its author had in view. Nothing under `frame/` may be written by a path nobody thought about, which is the same argument `LOCAL_ONLY_FRAME_FILES` makes one level up.

The barrier is **thread-local, not process-global**. `fr` runs one command on the main thread and no write in this crate happens on a spawned one, so the two are identical in the binary — but `cargo test` runs unit tests in parallel threads of one process, where a global would let one test arm the barrier and silently swallow the writes of every test beside it.

Three things it deliberately does not cover:

- **The lock files.** A dry run still takes `frame/.lock` and the two leaf locks, so a preview is computed against a project no other process is mid-write on. Creating an empty lock file is the one hole in "wrote nothing"; all three are in `LOCAL_ONLY_FRAME_FILES`.
- **Recovery.** `ops::recover` is skipped outright under a dry run rather than left to the barrier, because recovery is a write *followed by a re-read*: with the writes suppressed the reload returns the un-recovered project, silently dropping the repair from the copy the preview is computed against.
- **Registry auto-registration.** `load_project_at` touches the global registry on every command, `fr list` included. It is skipped rather than suppressed, so it stays out of the "would change" list — it is not an effect of the command being previewed, and naming it in every preview would teach people to stop reading the list.

Because the handler runs unchanged, a preview computes exactly what the real run computes — including the ID a `fr mv --track` would mint, which is the question the flag exists to answer. The one place the barrier is not transparent is a handler that reads back what it wrote: the three `fr track` commands that edit `project.toml` and then reload, which go through `after_config_write` instead.

`tests/parity.rs` holds the guard, in the shape of the `--json` one beside it: `DRY_RUN_SURFACE` declares a verdict per subcommand (a new one fails the build until classified), and `dry_run_writes_nothing` runs each row twice, asserting that the preview leaves `frame/` byte for byte *including `.ids.toml`*, that the real run does not, and that both exit the same way — the last being what catches a command that rejects the flag outright, as `fr mv` did.
...
**`project.toml` is a save target like any other.** It takes the lock, records a failure instead of discarding it, retries on the timer, is named in the unsaved indicator, and gets a copy in `.rescue/` at exit. It is merged like the other two, by the route above — `reconcile_config` applying our delta to the document on disk.

**A `project.toml` that cannot be read is not written.** Every `fr` command already refuses such a project: `load_project` fails, so the TUI will not start on one either. The save path used to be the exception — it fell back to `toml::to_string_pretty` of the in-memory struct, which models neither comments nor any key it does not know, so a file somebody was midway through resolving a merge conflict in was flattened to its settings, from a keystroke the user thought was a track rename. That is the same destruction the merge exists to prevent, arriving through the door the merge could not open, and the damaged text is the only copy of what they were writing.

So `save_config_locked` returns the error instead, which puts the config in `unsaved`: the retry re-runs against disk, so repairing the file by hand is the whole of what it takes for the held-up change to land, and the struct dump still reaches `frame/.rescue/project.toml` at exit if it never does. Beside the damaged file, not on top of it.

Three consequences follow from drawing that line, and each is the answer to a question the line raises.

A **missing** `project.toml` gets the opposite answer, because the test is whether a write destroys something and there is nothing there to destroy. Refusing would leave the project unloadable by every other command with the only config in one session's memory, and the retry could never clear itself. So it is rebuilt from the ancestor text — the file as it was when we last agreed with it — which is why the comments come back with it rather than a struct dump's worth of settings.

A **damaged one refuses the whole of a multi-file change, up front.** Every `App::with_project_lock` caller writes the config as one half of its change, so a refusal inside the body would leave the other half done: the track file moved with the config still calling it active — and an `.inflight` marker committed whose recovery asserts the config was already archived — or the only copy of a track unlinked from a config that still lists it. Asked once, under the lock that makes the answer hold, it needs no rollback and no reordering at any of the five sites.

**No ancestor is not a reason to replace the file.** With no baseline to compute a delta from, the merge takes *theirs* as the base: our keys win and their document — every comment, every key `ProjectConfig` does not model — is what gets written into. Same outcome as the struct dump on the keys we own, without the destruction.
