...
`ref:` and `spec:` are sets of file paths, so two sides adding to one is a union and a removal is honoured against the other side's additions — those two fields never conflict. `note:` is still one value: two writers appending to the same note in different words is the case no automatic answer is right for, and it conflicts. A task both sides edited comes out rewritten in canonical field order, because its lines are now neither side's.
