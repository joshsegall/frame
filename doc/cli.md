...
**A conflict is per field, not per task.** A task is a record of independent fields — state, title, tags, `added:`, `resolved:`, `dep:`, `ref:`, `spec:`, `note:` — and each merges on its own: changed by one side, take that side; changed identically by both, take it; changed differently by both, that field conflicts and the task with it. So finishing a task while somebody else annotates it is not a conflict, and neither is adding a `ref:` while they rewrite the note.

`ref:` and `spec:` are sets of file paths, so two sides adding to one is a union and a removal is honoured against the other side's additions — those two fields never conflict. A task both sides edited comes out rewritten in canonical field order, because its lines are now neither side's.

**A `note:` merges by its lines.** Notes accumulate: dated episodes at the tail, corrections above them. So one side appending while the other edits something forty lines up is not a disagreement, and it merges — a note conflicts only when the two sides' changed line ranges actually overlap, which is what two writers appending different text at the same point looks like. No conflict markers are ever written into a note; the file always parses. A task carrying two `- note:` lines is two notes, and those fall back to the whole-value rule rather than being spliced together.
...
So `project.toml` and `actors.toml` merge line by line for a different reason than you might expect: they are **never routed to the driver**, so nothing declines them — git simply merges them itself. That puts the burden on how those files are *written* rather than on a driver: `actors.toml` is serialized one line per actor and sorted by token so that git's own merge conflicts rarely and resolves safely when it does (see [format.md](format.md#actor-registry-frameactorstoml)). What declining is actually for is a file that *is* routed and whose shape frame cannot name; stopping is the right answer there, because reading a file as a shape it does not have is how data goes missing quietly.
