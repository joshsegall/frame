    ...
    match target {
        ...
        EditTarget::ImportFilePath { track_id } => {
            ...
            app.confirm_state = Some(super::app::ConfirmState {
                ...
                action: super::app::ConfirmAction::ImportTasks {
                    track_id,
                    file_path,
                },
            });
...
fn palette_archive_track(app: &mut App) {
    let track_id = match tracks_cursor_track_id(app) {
        Some(id) => id,
        None => return,
    };
...
fn palette_delete_track(app: &mut App) {
    let track_id = match tracks_cursor_track_id(app) {
        Some(id) => id,
        None => return,
    };
        ...
        (KeyModifiers::NONE, KeyCode::Char('y')) => {
            ...
            if let Some(state) = state {
                match state.action {
                    ...
                    super::app::ConfirmAction::DeleteTask { track_id, task_id } => {
                        confirm_delete_task(app, &track_id, &task_id);
                    }
                    super::app::ConfirmAction::BulkDeleteTasks { task_ids } => {
                        confirm_bulk_delete_tasks(app, &task_ids);
                    }
                    super::app::ConfirmAction::PruneRecovery => {
                        confirm_prune_recovery(app);
                    }
                    super::app::ConfirmAction::UnarchiveTrack { track_id } => {
                        confirm_unarchive_track(app, &track_id);
                    }
                    super::app::ConfirmAction::ImportTasks {
                        track_id,
                        file_path,
                    } => {
...
fn handle_results_overlay(app: &mut App, key: KeyEvent) {
    match (key.modifiers, key.code) {
        (_, KeyCode::Esc) | (_, KeyCode::Char('q')) => {
...
fn palette_check_project(app: &mut App) {
    use crate::ops::check;
...
fn palette_preview_clean(app: &mut App) {
    use crate::ops::clean;
    use ratatui::style::{Modifier, Style};
    use ratatui::text::{Line, Span};
...
fn palette_prune_recovery(app: &mut App) {
    ...
    app.confirm_state = Some(super::app::ConfirmState {
        message: msg,
        action: super::app::ConfirmAction::PruneRecovery,
    });
    app.mode = Mode::Confirm;
}

fn confirm_prune_recovery(app: &mut App) {
    use crate::io::recovery;
...
fn palette_unarchive_track(app: &mut App) {
    let track_id = match tracks_cursor_track_id(app) {
        Some(id) => id,
        None => return,
    };
    ...
    app.confirm_state = Some(super::app::ConfirmState {
        ...
        action: super::app::ConfirmAction::UnarchiveTrack { track_id },
    });
    app.mode = Mode::Confirm;
}

fn confirm_unarchive_track(app: &mut App, track_id: &str) {
    let track_name = app.track_name(track_id).to_string();
...
fn palette_import_tasks(app: &mut App) {
    let track_id = match app.current_track_id() {
        Some(id) => id.to_string(),
        None => return,
    };
...
fn confirm_import_tasks(app: &mut App, track_id: &str, file_path: &str) {
...
fn palette_delete_task(app: &mut App) {
    ...
    if !app.selection.is_empty() {
        ...
        app.confirm_state = Some(super::app::ConfirmState {
            message: msg,
            action: super::app::ConfirmAction::BulkDeleteTasks { task_ids },
        });
        app.mode = Mode::Confirm;
        return;
    }
    ...
    app.confirm_state = Some(super::app::ConfirmState {
        message: msg,
        action: super::app::ConfirmAction::DeleteTask { track_id, task_id },
    });
    app.mode = Mode::Confirm;
}

fn confirm_delete_task(app: &mut App, track_id: &str, task_id: &str) {
    use crate::io::recovery;
    use crate::ops::task_ops;
...
fn confirm_bulk_delete_tasks(app: &mut App, task_ids: &[(String, String)]) {
    use crate::io::recovery;
    use crate::ops::task_ops;
